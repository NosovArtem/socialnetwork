...
<div class="container">
    ...
    <form method="POST" action="${contextPath}/doUpload?${_csrf.parameterName}=${_csrf.token}"
        ...
        <p <%--class="form-heading"--%>>Select a photo to avtatara</p>
        <input type="file" <%--class="form-control"--%> name="fileUpload" size="50" placeholder="select avtatara"/>
        <button class="btn btn-primary btn-md"  type="submit">download</button>
    </form>



    <a href="${contextPath}/userinfo">Userinfo</a>
    <a href="${contextPath}/managefriends">Manage Friends</a>
            ...
            <c:forEach items="${listFriends}" var="userDTO">
                <tr>
                    <td><c:if test="${fn:length(userDTO.avatarBase64) gt 50}">
                        <img width="50px" src="${userDTO.avatarBase64}" alt="avatar"/>
                    </c:if></td>
                    <td><a href="${contextPath}/friends/${userDTO.id}">${userDTO.username}</a></td>
                </tr>
            </c:forEach>
        </table>
    </c:if>
